...
text(x=rda_sc2[5,1],
    ...
    'Massive  HC',
     cex=0.8,
     col='black')
text(x=rda_sc2[6,1]-0.11,
     y=rda_sc2[6,2],
     row.names(rda_sc2)[6],
     cex=0.75,
     col='black')
text(x=rda_sc2[7,1]-0.09,
     y=rda_sc2[7,2]-0.04,
     row.names(rda_sc2)[7],
     cex=0.75,
     col='black')
...
#######################################
#######OrdiR2Step Again###############################
fwd_sel<-ordiR2step(rda(rda_table[,c(15:25)]~1, data=rda_table[,1:14]), # lower model limit (simple!)
           scope = formula(rda_mod), # upper model limit (the "full" model)
           direction = "forward",
           R2scope = TRUE, # can't surpass the "full" model's R2
           pstep = 999,
           trace = FALSE)# change to TRUE to see the selection process!

rda_mod_summary<-summary(fwd_sel) 
RsquareAdj(fwd_sel) #Adjusted R2 = 23.83%

ordiplot(fwd_sel,type='n',
         xlab=paste('RDA1, ', 
...
title(main='Redundancy Analysis',
      sub='Variable selection by foward selection')

rda_sc<-scores(fwd_sel,choice=1:2,
               display='sp',
               scaling=3)
...
rda_sc1<-scores(rda_mod,choice=1:2,
                display='site',
                scaling=3)
rda_sc1[which(rda_sc1<0)]<-rda_sc1[which(rda_sc1<0)]-0.06
rda_sc1[which(rda_sc1>0)]<-rda_sc1[which(rda_sc1>0)]+0.04
text(x=rda_sc[,1],
     y=rda_sc[,2],
     c('VRM - 4 cm', 'PLC - 4 cm', 'PROC - 32 cm', 'PLC - 32 cm',
       'PLC - 16 cm', 'D (1 - 2 cm)', 'D (2 - 4 cm)', 'D (8 - 16 cm)',
       'D (16 - 32 cm)', 'D (32 - 64 cm)',  'Sq'),
     cex=0.8, col= 'black')

rda_sc2<-scores(fwd_sel,choice=1:2,
                scaling=2,
                display = 'bp')
text(x=rda_sc2[1,1],
     y=rda_sc2[1,2],
...
ordihull(fwd_sel, 
         ...
         col=my_cols,
         border = NA)
fwd_aov<-anova.cca(fwd_sel, by='axis',
                   permutations = how(nperm=999))
fwd_aov
fwd_aov_term<-anova.cca(fwd_sel, by='term',
                        permutations = how(nperm=999))
fwd_aov_term
fwd_aov_all<-anova.cca(fwd_sel,permutations = how(nperm=999))
fwd_aov_all
