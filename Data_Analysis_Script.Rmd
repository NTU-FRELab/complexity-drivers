...
indicators<-c('S - 4 cm', 'TRI - 4 cm', 'VRM - 4 cm', 
              ...
              'Sq')
colnames(comp_sub_metrics)[30:53]<-indicators
...
comp_sub_hel<-decostand(composition_substrates[,3:29], method='hellinger')
...
text(vct.scores[2,1],vct.scores[2,2],
...
text(vct.scores[4,1],vct.scores[4,2],
...
text(vct.scores[8,1],vct.scores[8,2],
...
text(vct.scores[11,1],vct.scores[11,2],
...
text(vct.scores[13,1],vct.scores[13,2],
     row.names(vct.scores)[13],
     col='black', cex=0.8) #bushy OC
text(vct.scores[14,1],vct.scores[14,2],
...
text(vct.scores[19,1],vct.scores[19,2],
     ...
     col='black', cex=0.8)#clustered OC
...
text(vct.scores[21,1],vct.scores[21,2],
     ...
     col='black', cex=0.8) #ocenc
text(vct.scores[22,1],vct.scores[22,2],
     row.names(vct.scores)[22],
     col='black', cex=0.8) #spopap
text(vct.scores[23,1],vct.scores[23,2],
...
text(vct.scores[25,1],vct.scores[25,2],
     row.names(vct.scores)[25],
     col='black', cex=0.8) #BSS
...
library(rempsyc)
set.seed(123)
comp_sub_std<-decostand(composition_substrates[,3:29], 'standardize')
...
composition_all$Abiotic<-rowSums(composition_all[,28:29])
table<-composition_all[,-c(4:6, 8:21, 23:26, 28:29)]
colnames(table)[1:6]<-c('Site', 'Region', 'AS', 'CO', 'AN', 'ZO')
table$Site[25]<-'82.5K'
library(tidyr)
library(ggplot2)
...
```{r glmnet, message=F}
comp_sub_metrics[,3:29]<-decostand(comp_sub_metrics[,3:29], 'hel') #Use hellinger-transformed data to explain complexity
library(glmnet)
...
cv_V4<-cv.glmnet(y=comp_sub_metrics$`VRM - 4 cm`, 
                 x=as.matrix(comp_sub_metrics[,3:29]),
...
row.names(cv_coef)<-c('Intercept', 'AN', 'Arborescent HC',
                      ...
                      'AS', 'Fan OC', 'Encrusting ZO', 'Globular SP',
                      'Massive SP', 'Clustered OC', 'Encrusting OC',
...
levels(groups2)<-my_cols
rda_table<-rda_table[,-1]
rda_mod<-rda(rda_table[,c(14:24)]~., data=rda_table[,1:13])
rda_mod_summary<-summary(rda_mod)
...
RsquareAdj(rda_mod)
#Show biplot of RDA
ordiplot(rda_mod,type = 'p',
         xlab=paste('RDA1, ', 
                    round(rda_mod_summary$cont$importance[2,1]*100, 2), '%'),
         ylab=paste('RDA2, ',round(rda_mod_summary$cont$importance[2,2]*100, 2), '%'),
         xlim=c(-1,1),
         cex=0.8)
title(
